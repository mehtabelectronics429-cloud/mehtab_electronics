...
import { Plus, Star, Pencil, Archive, KeyRound, Search } from "lucide-react";
...
export default function EmployeesPage() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      toast.success(
        editing ? "Employee updated" : "Employee created with login",
      );
    },
  ...
  return (
    <div>
      ...
      <ConfirmDialog
        ...
        message={`Archive ${pendingDelete?.name}? Their dashboard access will be disabled.`}
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
      ...
      <div
        className={`mt-0.5 text-sm font-medium ${accent ? "text-emerald-300" : "text-white/85"}`}
      >
        {value}
      </div>
    </div>
