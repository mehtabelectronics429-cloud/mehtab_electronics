...
export default function LedgerPageInner() {
  ...
  const canWhatsapp = user && (can(user.role, "whatsapp.send") || can(user.role, "whatsapp.view"));
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.ledger({ page, limit: 20, customerId }),
  });
  ...
  const columns: ColumnDef<LedgerEntry>[] = [
    ...
    { accessorKey: "customer", header: "Customer" },
    {
      ...
      header: "Install",
      cell: (i) => <span className="text-white/50">{i.getValue<string>() || "—"}</span>,
    },
    {
      accessorKey: "type",
      header: "Type",
    ...
    },
    {
      ...
      header: "Amount",
      cell: (i) => (
        <span className={i.getValue<number>() < 0 ? "text-emerald-300" : "text-white"}>
    ...
    },
    {
      ...
      cell: (i) =>
        i.getValue<string>() === "approved" ? (
          <Badge className="border-emerald-400/30 bg-emerald-400/10 text-emerald-300">Approved</Badge>
        ) : (
          ...
          {canWhatsapp && (
            <>
              <button
                ...
                onClick={() => sendWa.mutate({ id: i.row.original.id, channel: "direct" })}
              ...
              <button
                ...
                onClick={() => sendWa.mutate({ id: i.row.original.id, channel: "business" })}
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Button variant="secondary" onClick={() => api.downloadLedger(customerId)}>
