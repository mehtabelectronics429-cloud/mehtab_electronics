...
export default function DashboardPage() {
  ...
  const adminStats = [
    {
      ...
      value: String(k.todayInstalls),
      icon: "CalendarClock",
      accent: "cyan",
    },
    {
      label: "Pending Installations",
      value: String(k.pendingInstalls),
      icon: "Clock",
      accent: "solar",
    },
    {
    ...
    {
      label: "Margin",
      value: `${((k.margin || 0) * 100).toFixed(0)}%`,
      icon: "Percent",
      accent: "energy",
    },
    {
    ...
    },
    {
      ...
      value: String(k.pendingApproval),
      icon: "ShieldQuestion",
      accent: "solar",
    },
    {
  ...
  const myProfit = myApproved.reduce(
    (s, i) => s + ((i.amount || 0) - (i.cost || 0)),
    0,
  );
  const myOutstanding = inv.reduce(
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={
          isAdmin
            ...
            : "Your assigned work and recent activity."
        }
      />
              ...
              inv.slice(0, 8).map((i) => (
                ...
                >
                  <div>
                    ...
                    <div className="text-xs text-white/40">
                      {i.date} · Paid {pkr(i.paid)} / {pkr(i.amount)}
                    </div>
                  </div>
                  <StatusBadge status={i.status} />
                </div>
      ...
      {!isAdmin && !isCashier && (
        <Card className="mt-6 p-5">
          <h3 className="text-sm font-medium text-white/80">
            My Upcoming Installations
          </h3>
          <div className="mt-4 space-y-2">
            {mine.length === 0 ? (
              <p className="text-sm text-white/40">
                No assigned installations.
              </p>
            ) : (
              mine.map((i) => (
                <div
