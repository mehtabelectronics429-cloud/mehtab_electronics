...
import { Plus, Search, Pencil, Archive, MapPin, BookOpen, BellRing, Download, MessageCircle } from "lucide-react";
import toast from "react-hot-toast";
import { PageHeader, StatusBadge } from "@/components/admin/ui/feedback";
import { Button, Input, Label, Textarea, Badge } from "@/components/admin/ui/primitives";
...
export default function CustomersPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    reset({ name: "", phone: "", whatsapp: "", address: "", mapUrl: "", notes: "" });
    setOpen(true);
  };
    ...
    () => [
      {
        ...
        cell: (i) => (
          <div>
            ...
            <div className="text-xs text-white/40">{i.row.original.address}</div>
          </div>
        ),
      },
      { accessorKey: "phone", header: "Phone" },
      {
        ...
        header: "Jobs",
        cell: (i) => <span className="text-white/60">{i.getValue<number>()}</span>,
      },
      {
        accessorKey: "balance",
        header: "Balance",
        cell: (i) => (
          <span className={i.getValue<number>() > 0 ? "text-amber-300" : "text-white/60"}>
  ...
  return (
    <div>
      ...
      <Modal
        ...
        title={ledgerData?.customer?.name ? `Ledger · ${ledgerData.customer.name}` : "Customer ledger"}
        wide
      >
        {ledgerLoading || !ledgerData ? (
          <div className="py-8 text-center text-sm text-white/40">Loading ledger…</div>
        ) : (
          <div className="space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <div className="text-sm text-white/50">{ledgerData.customer.address}</div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant="secondary"
                  onClick={() => remind.mutate({ id: ledgerData.customer.id, channel: "direct" })}
                >
                  <BellRing className="h-4 w-4" /> Direct WhatsApp
                </Button>
                <Button
                  onClick={() => remind.mutate({ id: ledgerData.customer.id, channel: "business" })}
                >
            ...
            <div>
              <h4 className="mb-2 text-xs uppercase tracking-wider text-white/40">Ledger entries</h4>
                  ...
                  ledgerData.ledger.map((l) => (
                    ...
                    >
                      <div>
                        ...
                        <span className="ml-2 text-xs text-white/40">{String(l.date).slice(0, 10)}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className={l.amount < 0 ? "text-emerald-300" : "text-white"}>{pkr(l.amount)}</span>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <h4 className="mb-2 text-xs uppercase tracking-wider text-white/40">Installations</h4>
                <div className="max-h-40 space-y-1 overflow-y-auto">
                  {ledgerData.installations.map((i) => (
                    <div key={i.id} className="rounded-lg border border-white/10 px-3 py-2 text-sm">
              ...
              <div>
                <h4 className="mb-2 text-xs uppercase tracking-wider text-white/40">Invoices</h4>
                <div className="max-h-40 space-y-1 overflow-y-auto">
