    ...
    <section className="relative py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-6 md:px-8">
        ...
        <p className="lead mt-5 max-w-xl">
          One accountable team from the first site visit to years of after-sales support — the way
          we&apos;ve worked across Punjab since 1996.
        </p>
            ...
            <Reveal key={s.step} delay={i * 0.1}>
              <div className="group flex h-full flex-col overflow-hidden rounded-lg border border-line/15 bg-surface/40 shadow-card transition-all hover:-translate-y-1 hover:border-brand/40">
                <div className="relative aspect-[16/10] overflow-hidden">
                  <SmartImage src={s.image} alt={s.title} className="h-full w-full" imgClassName="transition-transform duration-700 group-hover:scale-105" />
                  ...
                  <span className="absolute right-4 top-4 font-display text-3xl text-white/85">{s.step}</span>
                </div>
                <div className="flex flex-1 flex-col p-6">
                  <h3 className="font-display text-xl uppercase tracking-wide text-fg">{s.title}</h3>
