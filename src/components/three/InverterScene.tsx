...
function Rig({
  ...
  useFrame((state) => {
    ...
    const inside = clamp01(
      smoothstep(0.34, 0.5, p) - smoothstep(0.88, 0.98, p),
    );
    const reveal = clamp01(
      smoothstep(0.34, 0.46, p) - smoothstep(0.9, 0.98, p),
    );
    ...
    panels.current.forEach((m, i) => {
      ...
      const d = PANELS[i];
      m.position.set(
        d.pos[0] + d.dir[0] * e * d.off,
        d.pos[1] + d.dir[1] * e * d.off,
        d.pos[2] + d.dir[2] * e * d.off,
      );
    ...
    if (phase === "component") {
      ...
      cz =
        8 -
        4.5 * smoothstep(0.12, 0.22, p) -
        1.8 * smoothstep(0.22, 0.35, p) -
        0.95 * smoothstep(0.35, 0.48, p) +
        7.2 * smoothstep(0.86, 1, p);
    }
...
export default function InverterScene({
  progress,
}: {
  progress: MotionValue<number>;
}) {
