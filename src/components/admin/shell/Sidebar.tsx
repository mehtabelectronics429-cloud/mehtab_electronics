...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 flex flex-col border-r border-white/10 bg-[#080a12]/95 backdrop-blur-xl transition-[width,transform] duration-300",
        width,
        ...
        <nav className="flex-1 space-y-5 overflow-y-auto px-3 py-4">
          {groups.map((g) => (
            <div key={g}>
              {!collapsed && <div className="px-3 pb-2 text-[0.62rem] font-semibold uppercase tracking-wider text-white/30">{g}</div>}
                  ...
                  return (
                    <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)}
                      className={cn("group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-colors",
                        active ? "text-white" : "text-white/55 hover:bg-white/5 hover:text-white", collapsed && "justify-center")}>
                      {active && <motion.span layoutId="nav-active-admin" className="absolute inset-0 rounded-xl border border-cyan/25 bg-cyan/10" transition={{ type: "spring", stiffness: 400, damping: 32 }} />}
        ...
        <div className="border-t border-white/10 p-3">
          <button onClick={logout} className={cn("flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-white/55 transition-colors hover:bg-red-500/10 hover:text-red-300", collapsed && "justify-center")}>
