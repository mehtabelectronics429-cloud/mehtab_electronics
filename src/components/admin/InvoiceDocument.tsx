...
export default function InvoiceDocument({ invoice }: { invoice: Invoice }) {
  ...
  const Row = ({ label, value, bold }: { label: string; value: string; bold?: boolean }) => (
    <div className={`flex items-center justify-between gap-6 py-1 ${bold ? "font-bold" : ""}`}>
  ...
  return (
    <div id="invoice-sheet" className="mx-auto w-full max-w-3xl bg-white p-8 text-[13px] text-neutral-900 md:p-10">
      {/* header */}
      <div className="flex items-start justify-between">
        <div />
        <div className="flex items-start gap-3 text-right">
          <div className="pt-1">
            <div className="text-[10px] font-semibold tracking-widest text-neutral-500">{L.since}</div>
            <div className="mt-8 text-[10px] italic text-neutral-400">{L.tagline}</div>
          </div>
          <span className="grid h-20 w-20 place-items-center overflow-hidden rounded-md" style={{ background: red }}>
            <Image src={LOGO_MARK} alt="Mehtab Electronics" width={210} height={178} className="h-[80%] w-[80%] object-contain" />
          </span>
        </div>
      </div>

      <h1 className="-mt-6 font-display text-4xl font-bold uppercase" style={{ color: red }}>
        Invoice
      </h1>
      ...
      <div className="mt-4 flex items-start justify-between gap-8">
        ...
        <div className="w-52 shrink-0">
          <div className="border-b border-neutral-300 pb-1 text-[10px] uppercase tracking-widest text-neutral-500">Date</div>
          <div className="py-1 font-medium">{invoice.date}</div>
          <div className="mt-2 border-b border-neutral-300 pb-1 text-[10px] uppercase tracking-widest text-neutral-500">Invoice Number</div>
      ...
      <div className="mt-6 flex items-start justify-between gap-8">
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-wide text-neutral-500">Invoice To</div>
          <div className="mt-1 font-medium">{invoice.customer || "—"}</div>
          {invoice.customerAddress && <div className="text-neutral-600">{invoice.customerAddress}</div>}
          {invoice.customerPhone && <div className="text-neutral-600">{invoice.customerPhone}</div>}
        </div>
        <div className="w-52 shrink-0">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-neutral-500">Prepared By</div>
      ...
      <table className="mt-6 w-full border-collapse text-[12px]">
        ...
        <tbody>
          {items.length === 0 && (
            <tr><td colSpan={4} className="px-2 py-6 text-center text-neutral-400">No line items</td></tr>
          )}
          {items.map((it, i) => (
            <tr key={i} className="border-b border-neutral-200">
              <td className="px-2 py-1.5">{it.description}</td>
              <td className="px-2 py-1.5 text-right tabular-nums">{it.qty}</td>
              <td className="px-2 py-1.5 text-right tabular-nums">{money(it.unitPrice)}</td>
              <td className="px-2 py-1.5 text-right tabular-nums">{money(it.qty * it.unitPrice)}</td>
            </tr>
          ))}
        </tbody>
      </table>
