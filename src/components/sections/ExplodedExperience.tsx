...
const PHASE_CAPTION: Record<ExplodePhase, { eyebrow: string; text: string }> = {
  ...
  explode: { eyebrow: "Exploded view", text: "Watch it come apart — piece by piece." },
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-[radial-gradient(120%_120%_at_50%_-10%,#0b1120_0%,#04060B_60%)]">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-24 px-6 md:bottom-auto md:right-14 md:top-1/2 md:w-[24rem] md:-translate-y-1/2 md:px-0">
          <AnimatePresence mode="wait">
            {active ? (
              <motion.div key={`c-${state.index}`} initial={{ opacity: 0, y: 24, filter: "blur(6px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={{ opacity: 0, y: -24, filter: "blur(6px)" }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }} className="animated-border mx-auto max-w-md rounded-3xl glass p-6">
                {images?.[state.index] && (
                ...
                <div className="flex items-center justify-between">
                  <span className="grid h-11 w-11 place-items-center rounded-xl bg-white/5 text-cyan ring-1 ring-white/10"><Icon name={active.icon} className="h-5 w-5" /></span>
              ...
              <motion.div key={`p-${state.phase}`} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="mx-auto max-w-md text-center md:text-left">
    ...
    <section className="dark relative overflow-hidden bg-[radial-gradient(120%_120%_at_50%_-10%,#0b1120_0%,#04060B_60%)] py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-6 md:px-8">
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.08}><h2 className="display-lg mt-4 text-white">{title}</h2></Reveal>
          <Reveal delay={0.16}><p className="mt-5 text-lg leading-relaxed text-white/60">A look at the engineering inside — every component, and what it does for you.</p></Reveal>
        </div>
              ...
              <article className="group flex h-full flex-col overflow-hidden rounded-3xl glass hairline transition-all duration-500 hover:-translate-y-1.5 hover:border-cyan/30">
                {images?.[i] && (
                  <div className="relative aspect-[16/10] overflow-hidden">
                    <SmartImage src={images[i]} alt={part.title} className="absolute inset-0 h-full w-full transition-transform duration-700 group-hover:scale-105" />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#04060B] via-transparent to-transparent" />
                    <span className="absolute left-4 top-4 grid h-9 w-9 place-items-center rounded-lg bg-black/40 text-cyan ring-1 ring-white/10 backdrop-blur"><Icon name={part.icon} className="h-4 w-4" /></span>
                    <span className="absolute right-4 top-4 font-mono text-xs text-white/50">{String(i + 1).padStart(2, "0")}</span>
                  </div>
                )}
                <div className="flex flex-1 flex-col p-6">
                  <h3 className="font-display text-base text-white">{part.title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-white/60">{part.description}</p>
                  <div className="mt-4 space-y-2 border-t border-white/10 pt-4 text-xs">
                    <div className="flex items-center gap-2 text-white/70"><span className="h-1.5 w-1.5 rounded-full bg-energy" /> {part.benefits}</div>
                    <div className="flex items-center gap-2 font-mono uppercase tracking-widest text-cyan/80"><span className="h-1.5 w-1.5 rounded-full bg-cyan" /> {part.spec}</div>
                  </div>
                </div>
              </article>
            </Reveal>
