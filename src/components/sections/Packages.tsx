...
const ring: Record<string, string> = {
  cyan: "hover:border-cyan/40", solar: "hover:border-solar/40", electric: "hover:border-electric/40",
};

export default function Packages() {
  return (
    <section id="packages" className="relative mx-auto max-w-7xl px-6 py-28 md:px-8 md:py-40">
      <SectionHeading
        eyebrow="Packages"
        align="center"
        title={<>Start with a <span className="text-gradient">package</span>, tailored to you.</>}
        ...
        {PACKAGES.map((p, i) => (
          <Reveal key={p.name} delay={i * 0.1}>
            ...
            >
              ...
              <div className="mt-5 font-display text-2xl text-gradient">{p.price}</div>
              <ul className="mt-6 flex-1 space-y-3">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 text-sm text-fg/75">
              ...
              <div className="mt-8">
                <MagneticButton href="/contact" variant={p.featured ? "primary" : "ghost"} className="w-full justify-center">
                  Get this setup
                </MagneticButton>
              </div>
            </div>
