      ...
      <div className="mt-16 grid auto-rows-[minmax(180px,auto)] grid-cols-2 gap-4 md:grid-cols-4">
        {SERVICES.map((s, i) => (
          ...
          >
            <article
              className={cn(
                "sheen group relative flex h-full flex-col justify-between overflow-hidden rounded-3xl glass hairline p-6 transition-all duration-500 hover:-translate-y-1.5",
                accentGlow[s.accent],
              )}
            >
              {s.image && (
              ...
              <span
                className={cn(
                  "grid h-11 w-11 place-items-center rounded-xl bg-fg/5 ring-1 ring-fg/10",
                  accentText[s.accent],
                )}
              >
              ...
              <div className="mt-6">
                <h3 className="font-display text-sm leading-snug tracking-wide text-fg">
                  {s.title}
                </h3>
