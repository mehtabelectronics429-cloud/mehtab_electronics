      ...
      <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
        ...
        <Reveal delay={0.1}>
          <p className="max-w-sm text-sm text-fg/50">
            From single villas to 480kW farms and 128-camera corporate grids
        ...
        {PROJECTS.map((p, i) => (
          <Reveal key={p.title} delay={(i % 3) * 0.08}>
            <article className="sheen group relative aspect-[4/5] overflow-hidden rounded-3xl glass hairline">
              ...
              <span className="absolute left-4 top-4 rounded-full glass hairline px-3 py-1 text-[0.7rem] uppercase tracking-widest text-cyan">
                {p.tag}
              </span>
