...
export default function Process() {
  ...
  return (
    <section id="process" className="relative mx-auto max-w-7xl px-6 py-28 md:px-8 md:py-40">
      <SectionHeading
        eyebrow="How it works"
        align="center"
        title={<>From survey to <span className="text-gradient">switch-on</span>.</>}
        ...
        <div className="space-y-12">
          {PROCESS.map((p, i) => (
            <Reveal key={p.step} delay={i * 0.05}>
              <div className={`relative flex items-center gap-8 pl-16 md:pl-0 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
                ...
                <div className={`md:w-1/2 ${i % 2 === 0 ? "md:pl-12" : "md:pr-12 md:text-right"}`}>
                  <div className="group rounded-3xl glass hairline p-7 transition-all duration-500 hover:-translate-y-1 hover:border-cyan/30">
                    <div className={`flex items-center gap-3 ${i % 2 === 1 ? "md:flex-row-reverse" : ""}`}>
                      <span className="grid h-11 w-11 place-items-center rounded-xl bg-fg/5 text-cyan ring-1 ring-line/10">
                        <Icon name={p.icon} className="h-5 w-5" />
                      </span>
                      <span className="font-display text-3xl text-fg/15">{p.step}</span>
                    </div>
                    <h3 className="mt-5 font-display text-base text-fg">{p.title}</h3>
