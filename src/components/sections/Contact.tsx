        ...
        <div className="grid gap-12 lg:grid-cols-2">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href={COMPANY.phoneHref}
                className="sheen inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-electric to-cyan px-6 py-3 text-sm font-medium text-gray-800 shadow-glow-blue transition-transform duration-300 hover:scale-[1.03]"
              >
                <Phone className="h-4 w-4" /> Call now
              </a>
              <WhatsAppButton
                label="WhatsApp us"
                message={`Hello ${COMPANY.name}, I'd like a free site survey.`}
              />
            </div>
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2">
              {details.map((d) => (
                <Reveal key={d.label}>
                  <a
                    href={d.href}
                    className="flex h-full items-start gap-3 rounded-2xl glass hairline p-5 transition-colors duration-300 hover:border-cyan/30"
                  >
                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-fg/5 text-cyan ring-1 ring-fg/10">
                      <d.icon className="h-5 w-5" />
                    </span>
                    <div>
                      <div className="font-mono text-[0.7rem] uppercase tracking-widest text-fg/45">
                        {d.label}
                      </div>
            ...
            <div className="rounded-[2rem] glass hairline p-7 md:p-9">
              {sent ? (
                <div className="flex h-full min-h-[24rem] flex-col items-center justify-center text-center">
                  ...
                  <p className="mt-3 max-w-sm text-sm text-fg/60">
                    Thanks, {form.name || "there"}. Our team will reach out
                    within one business day to schedule your free site survey.
                  </p>
                  <button
                ...
                <form onSubmit={submit} className="space-y-5">
                  ...
                  <Field label="Service of interest">
                    ...
                    >
                      {SERVICES.map((s) => (
                        <option key={s.key} className="bg-surface">
                          {s.title}
                        </option>
                      ))}
                    </select>
                  ...
                  <Field label="Tell us about your project">
                    <textarea
                      ...
                      placeholder="Roof size, load, location, timeline…"
                      className="input resize-none"
                    />
                  </Field>
                  <button
                    type="submit"
                    className="sheen group flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-electric to-cyan px-8 py-4 text-sm font-medium text-gray-800 shadow-glow-blue transition-transform duration-300 hover:scale-[1.02]"
                  >
...
function Field({
  ...
  return (
    <label className="block">
      <span className="mb-2 block font-mono text-[0.7rem] uppercase tracking-widest text-fg/45">
        {label}
      </span>
      {children}
    </label>
